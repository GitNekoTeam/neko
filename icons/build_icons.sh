...
if ! declare -F load_linux_png &>/dev/null; then
  load_linux_png() {
    rsvg-convert -w 512 -h 512 "icons/${QUALITY}/neko_cnl.svg" -o "$1"
  }
fi

if ! declare -F load_windows_ico &>/dev/null; then
  load_windows_ico() {
    rsvg-convert -w 256 -h 256 "icons/${QUALITY}/neko_cnl.svg" -o "neko_256.png"
    convert "neko_256.png" -define icon:auto-resize=256,128,96,64,48,32,24,20,16 "$1"
    rm -f "neko_256.png"
  }
fi
...
build_darwin_main() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/resources/darwin/code.icns" ]]; then
    ...
    if [[ "$1" == "no-template" ]]; then
      rsvg-convert -w 1024 -h 1024 "icons/${QUALITY}/neko_cnl.svg" -o "code_1024.png"
    else
      rsvg-convert -w 655 -h 655 "icons/${QUALITY}/neko_cnl.svg" -o "code_logo.png"
...
build_darwin_types() { # {{{
  if [[ "$1" == "no-border" ]]; then
    rsvg-convert -w 128 -h 128 "icons/${QUALITY}/neko_cnl.svg" -o "code_logo.png"
  else
    rsvg-convert -w 128 -h 128 "icons/${QUALITY}/neko_cnl_w80_b8.svg" -o "code_logo.png"
  fi
...
build_media() { # {{{
  if [[ ! -f "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg" ]]; then
    ...
    cp "icons/${QUALITY}/neko_clt.svg" "${SRC_PREFIX}src/${QUALITY}/src/vs/workbench/browser/media/code-icon.svg"
...
build_windows_type() { # {{{
  ...
  if [[ ! -f "${FILE_PATH}" ]]; then
    ...
    rsvg-convert -w "${LOGO_SIZE}" -h "${LOGO_SIZE}" "icons/${QUALITY}/neko_cnl.svg" -o "code_logo.png"
...
build_windows_types() { # {{{
  ...
  rsvg-convert -b "#F5F6F7" -w 64 -h 64 "icons/${QUALITY}/neko_cnl.svg" -o "code_logo.png"
